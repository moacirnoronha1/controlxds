...
BEGIN
  ...
  -- quantidades liberadas: usa o informado, nunca negativo, padrão = solicitada
  ...
  -- 1) reconcilia divergência entre saldo do produto e soma dos lotes, e valida tudo
  FOR r IN
  ...
  LOOP
    ...
    -- se houver lotes, a verdade é a soma dos lotes: acerta o saldo do produto
    IF v_lotes > 0 AND v_produto <> v_lotes THEN
      UPDATE public.produtos
         SET estoque_atual = v_lotes, updated_at = now()
       WHERE id = r.produto_id;
      v_produto := v_lotes;
    END IF;

    IF v_produto < r.qtd THEN
      RAISE EXCEPTION 'Estoque insuficiente para %. Disponível: %, liberado: %', r.nome, v_produto, r.qtd;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    SELECT EXISTS (
      SELECT 1 FROM public.lotes WHERE produto_id = r.produto_id AND saldo > 0
    ) INTO v_tem_lotes;
    ...
    IF v_restante > 0 THEN
      IF v_tem_lotes THEN
        -- havia lotes mas não cobriram: não baixa parcialmente, desfaz tudo
        RAISE EXCEPTION 'Lotes insuficientes para concluir a baixa do produto. Nada foi alterado.';
      END IF;
      -- produto sem lotes: baixa direto do saldo
