...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.momento = 'depois_chegada' AND COALESCE(NEW.quantidade, 0) > 0 THEN
    ...
    SELECT l.produto_id, l.local_id, l.saldo, l.validade, l.custo_unitario, p.nome
      INTO v_lote_produto, v_lote_local, v_saldo, v_validade, v_custo, v_produto_nome
      FROM public.lotes l
    ...
    END IF;
    IF round(COALESCE(v_saldo, 0), 6) < round(NEW.quantidade, 6) THEN
    ...
    VALUES (
      ...
      NEW.quantidade,
      'AVARIA ' || NEW.id::text
        || ' | Produto: ' || COALESCE(v_produto_nome, NEW.produto_id::text)
        || ' | Lote: ' || NEW.lote_id::text
        || ' | Validade: ' || COALESCE(to_char(v_validade, 'DD/MM/YYYY'), 'sem validade')
        || ' | Quantidade: ' || NEW.quantidade::text
        || ' | Usuário: ' || COALESCE(NULLIF(NEW.responsavel, ''), 'não informado')
        || ' | Motivo: ' || COALESCE(NULLIF(NEW.motivo, ''), 'não informado'),
...
GRANT EXECUTE ON FUNCTION public.aplicar_avaria() TO service_role;
