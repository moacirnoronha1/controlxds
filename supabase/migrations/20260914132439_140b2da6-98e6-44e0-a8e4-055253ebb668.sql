ALTER TABLE public.requisicoes
  ...
  ADD COLUMN IF NOT EXISTS dado_teste boolean NOT NULL DEFAULT false;
...
CREATE INDEX IF NOT EXISTS requisicoes_arquivado_em_idx ON public.requisicoes (arquivado_em, data DESC);
CREATE INDEX IF NOT EXISTS inventarios_arquivado_em_idx ON public.inventarios (arquivado_em, created_at DESC);
CREATE INDEX IF NOT EXISTS emprestimos_arquivado_em_idx ON public.emprestimos (arquivado_em, data_emprestimo DESC);
CREATE INDEX IF NOT EXISTS avarias_arquivado_em_idx ON public.avarias (arquivado_em, data DESC);

CREATE OR REPLACE FUNCTION public.gx_mestre_atual()
RETURNS text
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF v_nome IS NULL THEN
    RAISE EXCEPTION 'Somente o Mestre pode executar esta operação';
  END IF;
  RETURN v_nome;
END;
$$;

REVOKE ALL ON FUNCTION public.gx_mestre_atual() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.gx_mestre_atual() TO service_role;

CREATE OR REPLACE FUNCTION public.resumo_uso_banco()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  v_usuario := public.gx_mestre_atual();
  ...
  FROM (
    ...
    WHERE n.nspname = 'public'
      AND c.relkind = 'r'
      AND c.relname IN (
        'movimentacoes','inventario_itens','requisicao_itens','produtos','lotes',
        'requisicoes','inventarios','emprestimos','avarias','entrada_alteracoes',
        'requisicao_alteracoes','notas_fiscais','sessoes'
      )
  ) x;
  ...
  SELECT jsonb_build_object(
    ...
    'avarias', (SELECT count(*) FROM public.avarias WHERE arquivado_em IS NULL AND status = 'resolvido' AND updated_at < now() - interval '180 days')
  ) INTO v_arquivaveis;
  ...
  SELECT jsonb_build_object(
    ...
    'avarias', (SELECT count(*) FROM public.avarias a WHERE a.dado_teste AND a.momento <> 'depois_chegada')
  ) INTO v_testes;
  ...
  SELECT jsonb_build_object(
    ...
    'itens_requisicao', COALESCE((SELECT sum(n - 1) FROM (SELECT count(*) n FROM public.requisicao_itens GROUP BY requisicao_id, produto_id, quantidade_solicitada HAVING count(*) > 1) d), 0)
  ) INTO v_duplicidades;

  RETURN jsonb_build_object(
    'database_bytes', v_total,
    'tabelas', v_tabelas,
    'arquivaveis', v_arquivaveis,
    'testes_elegiveis', v_testes,
    'duplicidades_suspeitas', v_duplicidades,
    'retencao_dias', 180,
    'arquivos_no_banco', false,
    'gerado_em', now()
  );
END;
$$;
...
CREATE OR REPLACE FUNCTION public.arquivar_dados_antigos(_dias integer DEFAULT 180)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SET arquivado_em = now(), arquivado_por = v_usuario
  WHERE arquivado_em IS NULL
    AND status IN ('liberada','cancelada')
    AND COALESCE(liberada_em, cancelada_em, data) < now() - make_interval(days => _dias);
  ...
  WHERE arquivado_em IS NULL
    AND status = 'fechado'
    AND COALESCE(fechado_em, created_at) < now() - make_interval(days => _dias);
  ...
  WHERE arquivado_em IS NULL
    ...
    AND COALESCE(data_devolucao::timestamp, created_at) < now() - make_interval(days => _dias);
  GET DIAGNOSTICS v_emp = ROW_COUNT;

  UPDATE public.avarias
  SET arquivado_em = now(), arquivado_por = v_usuario
  WHERE arquivado_em IS NULL
    AND status = 'resolvido'
    AND updated_at < now() - make_interval(days => _dias);
  GET DIAGNOSTICS v_ava = ROW_COUNT;
...
CREATE OR REPLACE FUNCTION public.limpar_dados_teste_seguros()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  INSERT INTO gx_req_teste
  SELECT r.id FROM public.requisicoes r
  WHERE r.dado_teste
    AND r.status IN ('pendente','cancelada')
    AND NOT EXISTS (SELECT 1 FROM public.requisicao_itens ri WHERE ri.requisicao_id=r.id AND COALESCE(ri.quantidade_liberada,0)>0);
  DELETE FROM public.requisicao_alteracoes WHERE requisicao_id IN (SELECT id FROM gx_req_teste);
  DELETE FROM public.requisicao_itens WHERE requisicao_id IN (SELECT id FROM gx_req_teste);
  DELETE FROM public.requisicoes WHERE id IN (SELECT id FROM gx_req_teste);
  GET DIAGNOSTICS v_req = ROW_COUNT;

  CREATE TEMP TABLE gx_inv_teste(id uuid PRIMARY KEY) ON COMMIT DROP;
  INSERT INTO gx_inv_teste
  SELECT i.id FROM public.inventarios i
  WHERE i.dado_teste AND i.status <> 'fechado'
    AND NOT EXISTS (SELECT 1 FROM public.inventario_itens ii WHERE ii.inventario_id=i.id AND ii.contagem_fisica IS NOT NULL);
  DELETE FROM public.inventario_itens WHERE inventario_id IN (SELECT id FROM gx_inv_teste);
  DELETE FROM public.inventarios WHERE id IN (SELECT id FROM gx_inv_teste);
  GET DIAGNOSTICS v_inv = ROW_COUNT;

  DELETE FROM public.emprestimos WHERE dado_teste AND produto_id IS NULL;
  GET DIAGNOSTICS v_emp = ROW_COUNT;

  DELETE FROM public.avarias WHERE dado_teste AND momento <> 'depois_chegada';
  GET DIAGNOSTICS v_ava = ROW_COUNT;
